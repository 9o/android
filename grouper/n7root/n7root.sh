...
if [[ -x /usr/bin/adb ]]
  ...
  echo "Please enter the directory (WITH TRAILING FORWARD SLASH) which contains adb"
  read adb
  adblocation="$adb/adb"
fi
fi
...
if [[ -x /usr/bin/fastboot ]]
  ...
  echo "Please enter the directory (WITH TRAILING FORWARD SLASH) which contains fastboot"
  read fb
